...
import { percentage } from "./carouselUtils";
...
export default function ChakraCarousel({ children, gap, autoPlay }) {
...
function Slider({
    ...
    useEffect(() => {
        if(autoPlay) {
            const timer = setTimeout(() => ticking && handleContinue(), 3e3)
    ...
    return (
        <>
            ...
            <Flex w={`${itemWidth}px`} mt={`${gap / 2}px`} mx="auto" hidden>
                <Button
